...
class DCIWeb_CPost_Shipping_For_WC_Shipping_Rates {
	...
	private function make_request_to_canada_post($api_server, $request_data) {
		$cpServer = "https://$api_server/rs/ship/price/";
		$results  = wp_remote_post( $cpServer, $request_data);
	...
	private function build_canada_post_api_request($weight, $destination) {
		...
		if ($general_options['api_username'] . $general_options['api_password'] == '' 
			|| $general_options[ 'api_server' ] = '' ) {
			return;
		}

		if ($general_options['canda_post_server'] == '' ) {
			return;
		}
		...
		return $this->make_request_to_canada_post($general_options['canda_post_server'], $request_data);	
	}
	...
	public function check_api_keys() {
		...
		$cpServer = 'https://' . $general_options['canda_post_server'] . '/rs/ship/service?country=CA';
		$response = wp_remote_post( $cpServer, $request_data);
